...
interface BDLucroRealProps {
  onDataChange?: (data: any) => void
  onSaveComplete?: () => void
}

export const BDLucroReal: React.FC<BDLucroRealProps> = ({ onDataChange, onSaveComplete }) => {
  // Refs para sincronização de rolagem
  const headerScrollRef = useRef<HTMLDivElement>(null)
  const bodyScrollRef = useRef<HTMLDivElement>(null)

 // Hook do contexto de segmentos
 const { segmentosLucroReal, editarSegmento, removerSegmento } = useSegmentos()
 
 // Debug: Log dos dados do contexto
 console.log('🔍 [DEBUG] BD-LucroReal - segmentosLucroReal:', segmentosLucroReal)
 console.log('🔍 [DEBUG] BD-LucroReal - Quantidade de segmentos:', segmentosLucroReal.length)
 
 // Log quando os segmentos mudarem
 useEffect(() => {
   console.log('🔍 [DEBUG] BD-LucroReal - segmentosLucroReal atualizados:', segmentosLucroReal.length, 'segmentos')
 }, [segmentosLucroReal])

 // Hook do contexto compartilhado
 const {
   segmentoTributacaoLucroReal: segmentoTributacao,
   setSegmentoTributacaoLucroReal: setSegmentoTributacao,
   impostoRendaData,
   selectedPeriodo,
  ...
  // Estado para armazenar segmentos originais para comparação
  ...
  // Estados auxiliares para controle de cursor e edição ativa
 ...
 const isTextTruncated = (text: string, maxWidth: number = 220) => {
  ...
  // Função para carregar dados persistidos - DESABILITADA para usar nomes completos
  const loadPersistedData = async () => {
    try {
      console.log('🆕 Usando dados padrão com nomes completos de Segmento/Tributação')
    } catch (error) {
      console.error('❌ Erro ao carregar dados salvos de Segmento/Tributação:', error)
 ...
 useEffect(() => {
  ...
  // useEffect para monitorar mudanças no modal de Imposto de Renda
 useEffect(() => {
 ...
 // useEffect para atualizar automaticamente irValorDeduzir quando selectedPeriodo muda
 useEffect(() => {
   if (segmentosLucroReal.length > 0) {
     const novoValorDeduzir = getValorDeduzirByPeriodo()
     console.log('🔄 [AUTO UPDATE] Período mudou para:', selectedPeriodo, 'Novo valor deduzir:', novoValorDeduzir)
     
     // Atualizar todos os segmentos com o novo valor deduzir
     segmentosLucroReal.forEach(segmento => {
       if (segmento.irValorDeduzir !== novoValorDeduzir) {
         console.log('🔄 [AUTO UPDATE] Atualizando segmento:', segmento.nome, 'de', segmento.irValorDeduzir, 'para', novoValorDeduzir)
         ...
         const dadosAtualizados = {
           lucroReal: {
             ...
             irValorDeduzir: novoValorDeduzir,
             csll: segmento.csll
           }
         }
         
         editarSegmento(segmento.id, dadosAtualizados)
       }
     })
   }
 }, [selectedPeriodo, getValorDeduzirByPeriodo, segmentosLucroReal, editarSegmento])
  ...
  const hasFieldChanged = (itemId: number, field: string, currentValue: string): boolean => {
  ...
  const initializeOriginalValue = (itemId: number, field: string, value: string) => {
    const key = `${itemId}-${field}`
    if (!(key in originalFieldValues)) {
      setOriginalFieldValues(prev => ({ ...prev, [key]: value }))
    }
  }

  // Funções auxiliares para controle de cursor
 ...
 const updateSegmentoTributacao = (itemId: string, field: string, value: string) => {
   console.log('🔍 [DEBUG updateSegmentoTributacao] INÍCIO - itemId:', itemId, 'field:', field, 'value:', `'${value}'`);
   
   const segmentoAtual = segmentosLucroReal.find(item => item.id === itemId)
   if (!segmentoAtual) {
     console.log('🔍 [DEBUG updateSegmentoTributacao] ❌ ERRO: Segmento não encontrado para ID:', itemId);
     return // Item não encontrado
   }
   
   console.log('🔍 [DEBUG updateSegmentoTributacao] ✅ Segmento encontrado:', segmentoAtual.nome);
   const previousValue = segmentoAtual[field]
   console.log('🔍 [DEBUG updateSegmentoTributacao] Valor anterior:', `'${previousValue}'`, 'Novo valor:', `'${value}'`);
   
   // Só atualizar se realmente houve mudança no valor
   if (previousValue !== value) {
     console.log('🔍 [DEBUG updateSegmentoTributacao] Valores diferentes, atualizando...');
     ...
     const dadosAtualizados = {
       lucroReal: {
         ...
         ir: field === 'ir' ? value : segmentoAtual.ir,
         irAdicional: field === 'irAdicional' ? value : segmentoAtual.irAdicional,
         irValorDeduzir: field === 'irValorDeduzir' ? value : segmentoAtual.irValorDeduzir,
         csll: field === 'csll' ? value : segmentoAtual.csll
       }
     }
     
     console.log('🔍 [DEBUG updateSegmentoTributacao] Dados para editarSegmento:', JSON.stringify(dadosAtualizados, null, 2));
     console.log('🔍 [DEBUG updateSegmentoTributacao] Chamando editarSegmento com ID:', itemId);
     
     editarSegmento(itemId, dadosAtualizados);
     
     console.log('🔍 [DEBUG updateSegmentoTributacao] editarSegmento executado');
     ...
     if (onDataChange) {
       onDataChange({
        segmentosLucroReal: segmentosLucroReal,
        hasChanges: true
      })
       console.log('🔍 [DEBUG updateSegmentoTributacao] onDataChange chamado');
     }
   } else {
     console.log('🔍 [DEBUG updateSegmentoTributacao] Valores iguais, não atualizando');
   }
   
   console.log('🔍 [DEBUG updateSegmentoTributacao] FIM');
 }
 ...
 // Função para mudança no campo Nome do Segmento
 ...
 const handleModalNameKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
  ...
  const handleSaveSegmentos = async () => {
    ...
    const { updatedLR } = addSegmentoToBothTables(newSegmento)
    ...
    // Aguardar a atualização do contexto e então atualizar os dados originais
    setTimeout(() => {
      setOriginalSegmentos(segmentosLucroReal)
      setIsDirty(false)
      onSaveComplete()
      console.log('➕ [DEBUG] BD-LucroReal - Dados originais atualizados após adição e isDirty definido como false')
    }, 100)
     ...
     scrollbar-gutter: stable;
        ...
        .table-row:hover {
          ...
          transition: background-color 0.15s ease;
		       ...
		       /* Reservar espaço para barra de rolagem */
       .scrollbar-spacing {
         padding-right: 6px;
       }
 ...
 {/* Cabeçalho da Tabela com barra de rolagem oculta sincronizada */}
        <div className="bg-[#F8FAFC] border-b border-[#E5E7EB] sticky top-0 z-10">
          <div 
            ref={headerScrollRef}
            className="overflow-x-auto hidden-scrollbar ml-[10px] mr-[10px] pl-[0px]"
          ...
          > 
            <div className="ml-[3px] pr-[0px]">
              <div className="w-full flex h-[48px] items-center">
                ...
                <div 
                  className="segmento-column text-left mr-[-15px] pr-[0px] font-semibold text-[#374151] text-[15px] font-inter flex items-center"
                  style={{
                ...
                <div className="flex items-center h-[48px]">
                  {/* Coluna PIS */}
                  <div 
                    className="flex-shrink-0 text-center mr-[-5px] font-semibold text-[#374151] font-inter text-[13px] flex items-center justify-center" 
                    style={{width: '65px', whiteSpace: 'nowrap', height: '48px'}}
                  >
                    PIS
                  </div>
                  ...
                  {/* Coluna COFINS */} 
                  <div  
                    className="flex-shrink-0 text-center font-semibold text-[#374151] pl-[30px] font-inter text-[13px] flex items-center justify-center"
                  ...
                  {/* Colunas do Imposto de Renda */}
                   <div  
                    className="flex-shrink-0 text-center font-semibold pl-[20px] text-[#374151] font-inter text-[14px] flex items-center justify-center" 
                    style={{width: '280px', whiteSpace: 'nowrap', height: '48px'}}
                  >
                  ...
                  <div 
                    className="flex-shrink-0 text-center font-semibold pl-[30px] text-[#374151] font-inter text-[14px] flex items-center justify-center" 
                    style={{width: '70px', whiteSpace: 'nowrap', height: '48px'}}
                  >
                    <div className="flex items-center justify-center w-full">
                      <div className="flex-shrink-0 text-center ml-[-10px]" style={{width: '70px'}} title="Contribuição Social sobre o Lucro Líquido">
                        CSLL
                      </div>
                    </div>
                  </div>
            ...
            <table className="w-full" style={{tableLayout: 'fixed'}}>
		      ...
		      {segmentosLucroReal
                 ...
                 .map((item, index) => (
                  ...
                  <tr key={item.id} className="hover:bg-[#F9FAFB] group relative after:content-[''] after:absolute after:bottom-0 after:left-[0px] after:right-[15px] after:h-[1px] after:bg-[#D8E0F0CC] last:after:hidden flex" style={{height: 'auto', minHeight: '48px'}}>
                    ...
                    <td className="flex items-center" style={{height: '48px'}}>
                      <div className="flex items-center">
                        ...
                        <div className="p-[8px] text-center flex-shrink-0 pr-[0px]" style={{width: '80px'}}>
                          <input
                           ...
                           onChange={(e) => {
                             ...
                             console.log('🔍 [EXCLUSÃO DEBUG] ===== PIS onChange =====');
                             console.log('🔍 [EXCLUSÃO DEBUG] Valor digitado:', `'${value}'`);
                             console.log('🔍 [EXCLUSÃO DEBUG] Valor atual item:', `'${item.pis}'`);
                             console.log('🔍 [EXCLUSÃO DEBUG] É exclusão?', value === '' || value.length < item.pis.length);
                             ...
                             if (value === '' || value.length < item.pis.length) {
                               console.log('🔍 [EXCLUSÃO DEBUG] ✅ PERMITINDO EXCLUSÃO');
                        ...
                        <div className="p-[8px] text-center flex-shrink-0 pr-[0px]" style={{width: '80px'}}>
                          <input
                           ...
                           onChange={(e) => {
                             ...
                             console.log('🔍 [EXCLUSÃO DEBUG] ===== COFINS onChange =====');
                             console.log('🔍 [EXCLUSÃO DEBUG] Valor digitado:', `'${value}'`);
                             console.log('🔍 [EXCLUSÃO DEBUG] Valor atual item:', `'${item.cofins}'`);
                             console.log('🔍 [EXCLUSÃO DEBUG] É exclusão?', value === '' || value.length < item.cofins.length);
                             ...
                             console.log('🔍 [EXCLUSÃO DEBUG] ===== COFINS onChange FIM =====');
                           }}
                           onBlur={(e) => {
                             const fieldKey = `${item.id}-cofins`;
                             const currentValue = e.target.value;
                        ...
                        <div className="p-[8px] text-center flex-shrink-0 ml-[0px]" style={{width: '275px'}}>
                          <div className="excel-input-group flex">

                            <div 
                              style={{width: '70px', minWidth: '70px', maxWidth: '70px'}}
                              className="uniform-input excel-input-first px-[6px] py-[6px] text-[#1F2937] font-inter text-[13px] text-center bg-[#F9FAFB] border border-[#E5E7EB] flex items-center justify-center cursor-not-allowed">
                              {item.ir}
                            </div>
                        ...
                        <div className="p-[0px] text-center flex-shrink-0 ml-[0px]" style={{width: '70px'}}>
                          <div className="excel-input-group flex">
                            <div 
							style={{width: '70px', minWidth: '70px', maxWidth: '70px'}}
							className="uniform-input px-[6px] py-[6px] border border-[#D1D5DB] rounded-[4px] text-[#1F2937] font-inter text-[13px] text-center bg-[#F9FAFB] flex items-center justify-center cursor-not-allowed">
                              {item.csll}
                            </div>
                        ...
                        {/* Coluna Ações */}
                        <div className="p-[0px] text-center flex-shrink-0 ml-[10px] mr-[0px] pr-[0px]" style={{width: '32px'}}>
                          <button
                            onClick={() => {
                          console.log('🗑️ [DEBUG] BD-LucroReal - Clique no botão de exclusão para segmento:', item.segmento, 'ID:', item.id)
                          console.log('🗑️ [DEBUG] BD-LucroReal - Objeto completo do segmento:', item)
                          setSegmentoToDelete(item)
                          setShowDeleteModal(true)
                          console.log('🗑️ [DEBUG] BD-LucroReal - Modal de exclusão aberto')
                        }}
   ...
   <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-[70]">
     <div className="bg-white rounded-[16px] shadow-2xl w-[450px] max-w-[90vw] p-[32px]">
      ...
      <ModalConfirmarExclusao
        ...
        onConfirm={() => {
          console.log('🗑️ [DEBUG] BD-LucroReal - Confirmação de exclusão para segmento:', segmentoToDelete?.segmento, 'ID:', segmentoToDelete?.id)
          console.log('🗑️ [DEBUG] BD-LucroReal - Objeto segmentoToDelete completo:', segmentoToDelete)
          if (segmentoToDelete) {
            console.log('🗑️ [DEBUG] BD-LucroReal - Chamando removeSegmentoFromBothTables com nome:', segmentoToDelete.segmento)
            removeSegmentoFromBothTables(segmentoToDelete.segmento)
            // Também remover do SegmentosContext usando o nome para encontrar o ID correto
            const segmentoNoContext = segmentosLucroReal.find(s => s.nome === segmentoToDelete.segmento)
            if (segmentoNoContext) {
              console.log('🗑️ [DEBUG] BD-LucroReal - Removendo do SegmentosContext, ID encontrado:', segmentoNoContext.id)
              removerSegmento(segmentoNoContext.id)
            }
            // Aguardar a atualização do contexto e então atualizar os dados originais
            setTimeout(() => {
              setOriginalSegmentos(segmentosLucroReal.filter(s => s.nome !== segmentoToDelete.segmento))
              setIsDirty(false)
              onSaveComplete()
              console.log('🗑️ [DEBUG] BD-LucroReal - Dados originais atualizados e isDirty definido como false')
            }, 100)
            console.log('🗑️ [DEBUG] BD-LucroReal - Remoção concluída')
          }
          setShowDeleteModal(false)
          setSegmentoToDelete(null)
        }}

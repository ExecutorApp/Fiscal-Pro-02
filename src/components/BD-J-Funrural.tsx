...
interface BDJFunruralProps {
  onDataChange?: (data: any) => void
  onSaveComplete?: () => void
}

export const BDJFunrural: React.FC<BDJFunruralProps> = ({ onDataChange, onSaveComplete }) => {
  // Estado para os dados da tabela (valores específicos para pessoa jurídica)
  const [Funrural, setFunrural] = useState([
    { id: 1, descricao: 'Previdência', percentual: '1,70%' },
    { id: 2, descricao: 'Acidente trabalho', percentual: '0,10%' },
    { id: 3, descricao: 'Senar', percentual: '0,25%' }
  ...
  // 🔧 CORREÇÃO: Função para notificar o pai sobre mudanças (ESSENCIAL para modal de confirmação)
  ...
  const loadPersistedData = async () => {
    try {
      ...
      const savedData = localStorage.getItem('FunruralJuridica_data')
      
      if (savedData) {
        const parsedData = JSON.parse(savedData)
        setFunrural(parsedData.Funrural)
        console.log('📊 Dados de Funrural Jurídica carregados do armazenamento local!')
      } else {
        console.log('🔧 Usando dados padrão de Funrural Jurídica - primeira vez')
      }
      
    } catch (error) {
      console.error('❌ Erro ao carregar dados salvos de Funrural Jurídica:', error)
      console.log('🔄 Usando dados padrão de Funrural Jurídica')
    }
  }
  ...
  const handleSaveFunrural = async () => {
    ...
    try {
      ...
      console.log('=== DADOS PARA PERSISTÊNCIA - Funrural Jurídica ===')
      console.log('Salvando alterações de Funrural Jurídica:', JSON.stringify(dataToSave, null, 2))
      
      // SALVAMENTO PERMANENTE NO NAVEGADOR
      localStorage.setItem('FunruralJuridica_data', JSON.stringify(dataToSave))
      
      console.log('💾 Dados de Funrural Jurídica salvos permanentemente no navegador!')
      
      setShowSuccessMessage(true)
      setIsDirty(false)
      
      // 🔧 CORREÇÃO: Notificar o componente pai que o salvamento foi concluído
      if (onSaveComplete) {
        onSaveComplete()
      }
      ...
      console.error('Erro ao salvar Funrural Jurídica:', error)
      alert('Erro ao salvar os dados de Funrural Jurídica. Verifique o console para mais detalhes.')
    }
  }
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="pl-[10px] pr-[20px] pt-[12px] pb-[12px] bg-[#F8FAFC] border-b border-[#E5E7EB] flex items-center justify-between">
          <h3 className="text-[16px] font-bold text-[#1F2937] font-inter">
            Produtor rural / Pessoa jurídica / Funrural
          </h3>
          ...
          <div className="bg-white rounded-[20px] shadow-2xl p-[32px] w-[480px] max-w-[90vw]">
            ...
            <div className="flex gap-[12px] justify-center">
              ...
              <button
                ...
                className="px-[20px] py-[12px] bg-[#DC2626] hover:bg-[#B91C1C] rounded-[12px] text-white font-semibold transition-colors text-[16px] font-inter"
              >
                Remover Funrural
              </button>
            </div>

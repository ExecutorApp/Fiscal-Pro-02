...
interface BDJEstadoICMSProps {
  onDataChange?: (data: any) => void
  onSaveComplete?: () => void
}

export const BDJEstadoICMS: React.FC<BDJEstadoICMSProps> = ({ onDataChange, onSaveComplete }) => {
  ...
  useEffect(() => {
    if (isGlobalDirty && onDataChange) {
      onDataChange({
        EstadoICMS: EstadoICMS,
        hasChanges: true
      })
    }
  }, [isGlobalDirty, EstadoICMS, onDataChange])
  ...
  const handleEstadoICMSPercentageBlur = (index: number, field: string, event: React.FocusEvent<HTMLInputElement>) => {
    ...
    // Usar apenas o contexto global - CORREÇÃO: usar a função correta
    updateEstadoICMS(index, field, formattedValue)
  }
  ...
  const updateEstadoICMS = (index: number, field: string, value: string) => {
    updateEstadoICMSGlobal(index, field, value)
  }
  ...
  const addEstadoICMS = () => {
    if (newEstadoICMS.estado.trim() && newEstadoICMS.percentual.trim()) {
      ...
      setNewEstadoICMS({ estado: '', percentual: '' })
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="pl-[10px] pr-[20px] pt-[12px] pb-[12px] bg-[#F8FAFC] border-b border-[#E5E7EB] flex items-center justify-between">
          <h3 className="text-[16px] font-bold text-[#1F2937] font-inter">
            Produtor rural / Pessoa jurídica / Estado e ICMS
          </h3>
  ...
  Salvar
        ...
        <div className="bg-[#F8FAFC] border-b border-[#E5E7EB] sticky top-0 z-10">
          <div className="flex w-full pr-[32px]"> 
            <div className="flex-1 text-left pl-[12px] pr-[12px] pt-[12px] pb-[12px] font-semibold text-[#374151] font-inter text-[14px]" style={{flexBasis: '85%'}}>Estado</div> 
        ...
        {/* Conteúdo Principal */}
        <div className="flex-1 pl-[12px] pr-[0px] pt-[0px] my-[5px] mr-[10px] modern-scrollbar overflow-auto">
          <table className="w-full">
            <tbody>
              {EstadoICMS.map((item, index) => (
                <tr key={item.id} className="hover:bg-[#F9FAFB] group relative after:content-[''] after:absolute after:bottom-0 after:left-[0px] after:right-[15px] after:h-[1px] after:bg-[#D8E0F0CC] last:after:hidden">
                  <td className="p-[0px] text-[#1F2937] font-inter text-left text-[15px]" style={{width: '85%'}}>{formatEstadoName(item.estado)}</td>
                  <td className="p-[12px] text-center" style={{width: '13%'}}>
                    <input
                      type="text"
                      value={item.percentual}
                      onChange={(e) => updateEstadoICMS(index, 'percentual', e.target.value)}

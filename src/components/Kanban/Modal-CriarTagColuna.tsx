...
interface Tag {
 ...
 export const ModalCriarTagColuna: React.FC<ModalCriarTagProps> = ({
   isOpen,
   onClose,
   onAddTag,
   columnId,
    ...
    if (color && !customColors.includes(color) && !predefinedColors.includes(color)) {
       ...
       try {
         const dataToSave = {
           ...
           version: "1.0",
		   columnId: columnId || 'global',
           customColors: updatedColors
         };
         const storageKey = columnId ? 
           `modal_criar_tag_coluna_custom_colors_${columnId}` : 
           'modal_criar_tag_coluna_custom_colors';
         localStorage.setItem(storageKey, JSON.stringify(dataToSave));
         console.log(`?? Cores personalizadas salvas no localStorage para coluna ${columnId || 'global'}!`);
   ...
   if (colorToDelete) {
       ...
       try {
         ...
         const storageKey = columnId ? 
           ...
           'modal_criar_tag_coluna_custom_colors';
         localStorage.setItem(storageKey, JSON.stringify(dataToSave));
         console.log(`??? Cor removida e localStorage atualizado para coluna ${columnId || 'global'}!`);
   ...
   const loadPersistedCustomColors = () => {
     try {
       ...
       if (savedData) {
         ...
         console.log(`🎨 Cores personalizadas carregadas do localStorage para coluna ${columnId || 'global'}!`);
       } else {
         console.log(`🆕 Primeira vez - sem cores personalizadas salvas para coluna ${columnId || 'global'}`);
       }
     } catch (error) {
       console.error(`? Erro ao carregar cores personalizadas para coluna ${columnId || 'global'}:`, error);
...
className="flex gap-2 overflow-x-auto scrollbar-hide py-3"
   ...
   {showDeleteConfirm && (
     <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-[90]">
 ...
 // Exportação com nome antigo para compatibilidade
 export const ModalCriarTag = ModalCriarTagColuna;
 
 export default ModalCriarTagColuna;
